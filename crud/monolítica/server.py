#Construyendo un CRUD monolítico RESTful caso: blog personal
...
db = {
  1: {
    ...
    "content": "¡Hola mundo! Esta es mi primera publicaión en el blog."
  },
  2: {
    "title": "Otra publicación",
    "content": "¡Bievenidos a mi blog! Aquí hay otra publicación,",
  },
}
...
class BlogHandler(BaseHTTPRequestHandler):
  ...
  def do_POST(self):
    ...
    else: self.send_error(404, "Ruta no válida")
  ...
  def do_PUT(self):
    ...
    if self.path.startswith("/post/"):
      ...
      if post_id in db:
        content_length = int(self.headers["Content-Length"])
        post_data = self.rfile.read(content_length)
        post_params = parse_qs(post_data.encode())
